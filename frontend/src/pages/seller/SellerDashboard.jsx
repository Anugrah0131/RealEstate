        ...
        const fetchData = async () => {
            try {
                const [statsRes, propsRes, inqRes] = await Promise.all([
                    ...
                    axios.get(`${API_URL}/api/inquiries/seller`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                ]);
                setStats((statsRes && statsRes.data && statsRes.data.stats) || statsRes.data || {});
                const props = Array.isArray(propsRes.data)
                    ? propsRes.data
                    : (propsRes.data && propsRes.data.properties) || [];
                setProperties(props);
                const inqData = inqRes.data;
                const inqList = Array.isArray(inqData.inquiries)
                    ? inqData.inquiries
                                ...
                                <div className={s.inquiryLeft}>
                                    ...
                                    <div>
                                        <div className={s.inquiryName}>
                                            {inq.properties?.title?.length > 30
                                                ? inq.properties?.title?.slice(0, 30) + "..."
